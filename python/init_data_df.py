...
def evaluate_high_symmetry_path(args, cell):
    if args.print_high_symmetry_points:
        comm.print_high_symmetry_points(cell, args)
        exit(0)

    if args.high_symmetry_path is not None:
        try:
            comm.check_high_symmetry_path(cell, args)
            ...
            exit(-1)
    if args.high_symmetry_path is not None:
       kmesh_hs, Hk_hs, Sk_hs = comm.high_symmetry_path(cell, args)
       inp_data = h5py.File(args.output_path, "a")
       print(kmesh_hs)
       print(cell.get_scaled_kpts(kmesh_hs))
       inp_data["high_symm_path/k_mesh"] = cell.get_scaled_kpts(kmesh_hs)
       inp_data["high_symm_path/r_mesh"] = comm.construct_rmesh(args.nk, args.nk, args.nk)
       inp_data["high_symm_path/Hk"] = Hk_hs
       inp_data["high_symm_path/Sk"] = Sk_hs

def evaluate_second_order_ewald(args, mycell):
    ...
    comm.compute_ewald_correction(args, mycell, kmesh, args.hf_int_path + "/df_ewald.h5")

def init_input_data(args, cell):
...
if args.job == "init":
    init_input_data(args, mycell)
elif args.job == "sym_path":
    evaluate_high_symmetry_path(args, mycell)
elif args.job == "ewald_corr":
    evaluate_second_order_ewald(args, mycell)
