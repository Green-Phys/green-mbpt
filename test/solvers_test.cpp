...
TEST_CASE("MBPT Solver") {
  SECTION("HF") {
    ...
    std::string df_int_path = TEST_PATH + "/HF/df_hf_int"s;
    std::string test_file   = TEST_PATH + "/HF/data.h5"s;
    ...
    green::sc::ztensor<4>                 tmp;
    {
      ...
      tmp << S_k.view<std::complex<double>>().reshape(ns, nk, nao, nao);
    }
    {
    ...
    auto Sigma1_test = green::sc::ztensor<4>(ns, ink, nao, nao);
  ...
  SECTION("GW") {
    ...
    std::string df_int_path = TEST_PATH + "/GW/df_int"s;
    std::string test_file   = TEST_PATH + "/GW/data.h5"s;
    std::string grid_file   = GRID_PATH + "/ir/1e4.h5"s;
    std::string args =
        "test --restart 0 --itermax 1 --E_thr 1e-13 --mixing_type SIGMA_DAMPING --damping 0.8 --input_file=" + input_file +
    ...
    auto G_shared     = green::utils::shared_object(green::sc::ztensor<5>(nullptr, nts, ns, ink, nao, nao));
    auto S_shared     = green::utils::shared_object(green::sc::ztensor<5>(nullptr, nts, ns, ink, nao, nao));
    auto S_shared_tst = green::utils::shared_object(green::sc::ztensor<5>(nullptr, nts, ns, ink, nao, nao));
    auto Sigma1       = green::sc::ztensor<4>(ns, ink, nao, nao);
    auto Sk           = green::sc::ztensor<4>(ns, ink, nao, nao);
    Sk(0) << bz.full_to_ibz(tmp(0));
    Sk(1) << bz.full_to_ibz(tmp(1));
